...
def algorithme_genetique_basique(target, mutation_rate, num_offspring=128, max_iterations=None):
    # Obtenir la longueur du mot cible
    target_length = len(target)
    
    # Initialiser le parent avec un mot aléatoire de la même longueur que le mot cible
    parent = generate_random_word(target_length)
    ...
    while max_iterations is None or step < max_iterations:
        ...
        # Générer une liste d'enfants en mutant le parent
        offspring = [mutate_word(parent, mutation_rate) for _ in range(num_offspring)]

        # Sélectionner le meilleur enfant en fonction de son aptitude par rapport au mot cible
        best_offspring = max(offspring, key=lambda word: calculate_apte(word, target))
        apte_parent = calculate_apte(best_offspring, target)

        # Si l'un des enfants est au moins aussi apte que le mot cible, l'algorithme a trouvé la solution
        if apte_parent >= target_length:
            print(f"Étape {step}: Meilleur mot = '{best_offspring}' (Apte = {apte_parent})")
            print(f"Mot cible trouvé par l'algorithme génétique basique à la génération {step}!")
            break
        else:
            # Mettre à jour le parent avec le meilleur enfant pour la prochaine génération
            parent = best_offspring  

        # Afficher l'état actuel
        print(f"Étape {step}: Meilleur mot = '{parent}' (Apte = {apte_parent})")
...
def main():
    ...
    if search_option == "1":
        ...
        algorithme_genetique_basique(target_word, mutation_rate, population_size, max_iterations)
